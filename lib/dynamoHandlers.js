...
DynamoStore.prototype.find = function(request, callback) {
  ...
  self.dynamodb.query(params, function(err, data) {
    ...
    DynamoStore._inflateObject(result);
    console.log(result);
...
DynamoStore.prototype.create = function(request, newResource, callback) {
  ...
  var params = {
    ...
    Item: DynamoStore._deflateObject(newResource).M
  };
  ...
  self.dynamodb.putItem(params, function(err, data) {
    ...
    // console.log("Added item:", JSON.stringify(arguments, null, 2));
...
DynamoStore._deflateObject = function(someObject) {
  var newObject = { M: { } };

  Object.keys(someObject).forEach(function(attributeName) {
    var value = someObject[attributeName];
    if (typeof value === "boolean") {
      newObject.M[attributeName] = { BOOL: value };
    } else if (typeof value === "string") {
      newObject.M[attributeName] = { S: value };
    } else if (typeof value === "number") {
      newObject.M[attributeName] = { N: ("" + value) };
    } else if (value instanceof Array) {
      newObject.M[attributeName] = { L: value.map(DynamoStore._deflateObject) };
    } else if (value instanceof Object) {
      newObject.M[attributeName] = DynamoStore._deflateObject(value);
    } else {
      throw new Error("What type is: " + value);
    }
  });
...
DynamoStore._inflateObject = function(someObject) {
  for (var i in someObject) {
    var type = Object.keys(someObject[i])[0];
    if ([ "BOOL", "S", "N" ].indexOf(type) !== -1) {
      someObject[i] = someObject[i][type];
    } else if (type === "L") {
      someObject[i] = someObject[i].L.map(function(l) {
        return DynamoStore._inflateObject(l.M);
      });
    } else if (type === "M") {
      someObject[i] = DynamoStore._inflateObject(someObject[i][type]);
    } else {
