...
joiConverter.simpleAttribute = (attribute, joiScheme, write) => {
  ...
  if (type === 'array') {
    ...
    const innerType = joiConverter.simpleAttribute(attribute, joiScheme._inner.items[0], write)
    ...
    joiScheme._inner.children.forEach(attr => {
      fields[attr.key] = {
        ...
        type: joiConverter.simpleAttribute(`${attribute}_${attr.key}`, attr.schema, write)
      }
    })
    ...
    type = new ObjType({
      name: (write ? 'write' : 'read') + attribute,
...
joiConverter.swap = (attribute, joiScheme, graphQlResources) => {
  let type
  if (!joiScheme._settings) {
    type = joiConverter.simpleAttribute(attribute, joiScheme)
  } else {
...
joiConverter.shallowInput = (attribute, joiScheme) => {
  let type
  if (!joiScheme._settings) {
    type = joiConverter.simpleAttribute(attribute, joiScheme, true)
  } else {
