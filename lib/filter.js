  ...
  for (var i in request.params.filter) {
    ...
    if (!request.resourceConfig.attributes[i]) {
      return callback({
        ...
        detail: request.resourceConfig.resource + " do not have attribute or relationship '" + i + "'"
      });
    }
    ...
    if (relationSettings && relationSettings.__as) {
      return callback({
        ...
        code: "EFORBIDDEN",
        title: "Invalid filter",
        detail: "Filter relationship '" + i + "' is a foreign reference and does not exist on " + request.resourceConfig.resource
      });
    }
  }
