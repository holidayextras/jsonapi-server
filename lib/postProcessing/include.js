...
includePP.action = function(request, response, callback) {
  ...
  includes = includes.map(includePP._expandInclusionString);
...
includePP._expandInclusionString = function(includeString) {
  return includeString.replace(/([^.\]]*?|\[.*?\])\[\d+\]/ig, function(match) {
    // match ~~ articles[4]
    var parts = match.match(/\[?(.*?)\]?\[(\d+)\]/);
    // parts ~~ articles / 4
    var expandedString = parts[1];
    var repetition = parseInt(parts[2], 10);
    if (repetition > 10) repetition = 10;
    for (var i = repetition; i > 1; i--) {
      expandedString += "." + parts[1];
    }
