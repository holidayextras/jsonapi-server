...
jsonApi.define = resourceConfig => {
  ...
  resourceConfig.onUpdate = _.pick.apply(_, [].concat(resourceConfig.attributes, Object.keys(resourceConfig.attributes).filter(i => (resourceConfig.attributes[i]._meta.indexOf('readonly') === -1) && (resourceConfig.attributes[i]._meta.indexOf('immutable') === -1) && (!(resourceConfig.attributes[i]._settings || { }).__as))))
}
