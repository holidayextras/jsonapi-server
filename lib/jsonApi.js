...
jsonApi.setConfig = apiConfig => {
  ...
  jsonApi._apiConfig.generateId = jsonApi._apiConfig.generateId || uuid.v4
  jsonApi._apiConfig.pageLimit = jsonApi._apiConfig.pageLimit || 50
  
  responseHelper.setBaseUrl(jsonApi._apiConfig.pathPrefix)
  responseHelper.setMetadata(jsonApi._apiConfig.meta)
  responseHelper.setValidateOutput(!jsonApi._apiConfig.doNotValidateOutput)
}
...
jsonApi.define = resourceConfig => {
  ...
  const attributes = {}
  for (const key in resourceConfig.attributes) {
    if (!resourceConfig.foreignIncludes.includes(key)) {
      attributes[key] = resourceConfig.attributes[key]
    }
  }
  resourceConfig.foreignAttributes = attributes
