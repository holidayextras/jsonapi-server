...
responseHelper.setValidateOutput = validate => {
  responseHelper._validateOutput = validate
}
...
responseHelper._enforceSchemaOnArray = (items, schema, callback) => {
  ...
  if (responseHelper._validateOutput) {
    ...
    return callback(null,
      items.filter(
        ...
        item => responseHelper._generateDataItem(item, schema)
      )
    )
  }
}
...
responseHelper._enforceSchemaOnObject = (item, schema, callback) => {
  // TODO: find another way, JSON.stringify will be called even if the log is thrown away
  // maybe encapsulate inside a function that get called if we log ??
  ...
  if (responseHelper._validateOutput) {
    ...
    const dataItem = responseHelper._generateDataItem(item, schema)
    return callback(null, dataItem)
  }
}
...
responseHelper._generateLink = (item, schemaProperty, linkProperty) => {
  ...
  if (schemaProperty._settings.__one) {
    ...
    if (linkItem) {
      const linkItemObject = Array.isArray(linkItem) ? linkItem[0] : linkItem
      // $FlowFixMe: the data property can be either undefined (not present), null or [ ]
      if (linkItemObject) {
        link.data = {
          type: linkItemObject.type,
          id: linkItemObject.id,
          meta: linkItemObject.meta
        }
        link.links.related = `${responseHelper._baseUrl}${linkItemObject.type}/${linkItemObject.id}`
      }
    }
  ...
  if (schemaProperty._settings.__as) {
    ...
    if (schemaProperty._settings.__one && !(link.data && link.data.id && link.data.type)) {
      link.data = null
    }
    if (!link.data) {
