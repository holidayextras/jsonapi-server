...
responseHelper._generateLink = (item, schemaProperty, linkProperty) => {
  ...
  if (schemaProperty._settings.__many) {
    ...
    // delete data property if the linkItems are explicitly undefined (as opposed to null).
    if (linkItems === undefined) { delete link.data }
    if (linkItems) {
      link.data = [ ]
  ...
  if (schemaProperty._settings.__one) {
    ...
    // delete data property if the linkItems are explicitly undefined (as opposed to null).
    if (linkItem === undefined) { delete link.data }
    if (linkItem) {
      link.data = {
        type: linkItem.type,
        id: linkItem.id,
